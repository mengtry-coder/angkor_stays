                   ...
                   // get rate plan 
                    ...
                    if($get_rate_plan){
                        ...
                        foreach($get_rate_plan as $rp){
                            ...
                            // $checkRateIndex = Rate::find()->where(['=','rate_plan_id',32])->where(['=','accommodation_id',49])->where(['=','date','2020-08-04'])->All();
                            // print_r($checkRateIndex);
                    ...
                    $date_av = $row['date']; 
                    $av_val = $row['number_of_guest'];
                    // check rate========
                    $checkRate = Rate::find()->where(['=','accommodation_id',$accommodation_id])->andWhere(['>','price',0])->andWhere(['=','date',$date_av,])->All();
                    echo $rateChoice =  sizeof($checkRate);
                    // check if get rate data size < $numberOfNights
                    echo $numberOfNights;
                    if($rateChoice < $numberOfNights){
                        $rateAvailable = 0; // 0 is false mean have no rate some day or < 1
                    }else{
                        $rateAvailable = 1; // 1 is true mean have rate each day
                    }
                    // echo $rateAvailable;
                    if ($av_val <= 0 || $rateAvailable == 0) {
                        $row['accommodation_id'];
                        $query->andFilterWhere(['<>', 'accommodation_profile.id', $row['accommodation_id']]);
                        // $id_no_in_allotment = Yii::$app->db->createCommand("SELECT id FROM accommodation_profile WHERE id NOT IN (SELECT accommodation_id FROM allotment)")->queryAll();
                        // $query->andFilterWhere(['NOT IN', 'accommodation_profile.id', $id_no_in_allotment]);
                    }
                }
                // echo $query->createCommand()->getRawSql();
                exit();
            }else{
                $not_in_allotment = Yii::$app->db->createCommand("SELECT * from accommodation_profile 
                                where id not in (select accommodation_id from allotment where date between '$this->from_date' AND '$this->from_date')")->queryAll();
                foreach ($not_in_allotment as $key => $value) {
                     $query->andFilterWhere(['<>', 'accommodation_profile.id', $value['id']]);
                }
