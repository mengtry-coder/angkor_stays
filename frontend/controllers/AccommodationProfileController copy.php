...
class AccommodationProfileController extends Controller
{
    ...
    public function actionDependent()
    {
                ...
                if ($val_res == 0) {
                    ...
                    $numberOfNights= $date2->diff($date1)->format("%a");
                        ...
                        $response = Yii::$app->db->createCommand("
                            SELECT rp.name as name, rp.id as id, rp.guest as guest_number, r.price * $numberOfNights as price, r.promotion as promotion, r.promotion_type as promotion_type
                            from rate as r
        ...
        if(Yii::$app->request->post('action') == 'get_acc_profile'){
            ...
            if ($acc_type_id) {
                $response = AccommodationProfile::find()->select(['feature_image', 'id','name', 'LatitudeLongitude', 'address'])->where(['status' => 1, 'accommodation_type' => $acc_type_id])->asArray()->all();
            return json_encode($response);

            }else{
                $response = AccommodationProfile::find()->select(['feature_image', 'id','name', 'LatitudeLongitude', 'address'])->where(['status' => 1])->asArray()->all();
            return json_encode($response);
