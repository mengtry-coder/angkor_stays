...
class AllotmentController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        if ($searchModel->year && $searchModel->month) {
            ...
            // $filter_guest = $searchModel->guest;
            // $filter_to_date = $searchModel->to_date;
    ...
    {
        $id_month = $month;
        $id_year = YearList::find()->where(['name'=>$year])->one()->id;
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            $model->status = 1;
            $deleteMysql = "DELETE from allotment
            where date BETWEEN '$model->from_date' AND '$model->to_date' AND accommodation_id = $acc_id";
            // exit();
                $str= "";
                for ($i=$model->from_date; $i <= $model->to_date; $i++) {
                        $str = "('$i', '$acc_id', $model->number_of_guest, '$updated_date', $updated_by, 1)".",".$str;
                    }
                    $str = rtrim($str, ",");
                    $insert = "INSERT INTO allotment(date, accommodation_id, number_of_guest, updated_date, updated_by, status) VALUES". $str;
                    // print_r($insert);
                    // exit();
                    Yii::$app->db->createCommand("$deleteMysql")->execute();
        }
        if(Yii::$app->db->createCommand($insert)->execute()){
            Yii::$app->session->setFlash('success', "Saved successful");
            return $this->redirect('index.php?r=allotment%2Findex&AllotmentSearch%5Bmonth%5D='.$id_month.'&AllotmentSearch%5Byear%5D='.$id_year.'&AllotmentSearch%5Baccommodation_id%5D='.$acc_id.'');
